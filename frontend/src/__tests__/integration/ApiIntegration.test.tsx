...
const DashboardIntegration = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // Simulate loading multiple API endpoints - usando endpoints válidos
      ...
      const healthData = await healthResponse.json(); // Mudado de statusData para healthData
...
const ApiHookIntegration = () => {
  ...
  const statusApi = useApi(async () => {
    const response = await fetch('/api/health'); // Mudado de /api/status para /api/health
    const data = await response.json();
    if (!data.success) throw new Error(data.error || 'API Error');
    return data.data;
  });
  ...
  describe('Dashboard Integration', () => {
    ...
    it('should handle complete API failure', async () => {
      ...
      server.use(
        ...
        http.get('/api/health', () => HttpResponse.error()), // Mudado de /api/status para /api/health
