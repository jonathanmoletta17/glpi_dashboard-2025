...
>(({ item, level = 0, index, totalItems }, ref) => {
  ...
  const { handleKeyDown: handleNativeKeyDown } = useKeyboardNavigation({
    ...
    onSpace: handleClick,
    onArrowRight:
      hasChildren && orientation === 'horizontal' ? () => setIsExpanded(true) : undefined,
    onArrowLeft:
      hasChildren && orientation === 'horizontal' ? () => setIsExpanded(false) : undefined,
        ...
        <ul
          ...
          className={cn(
            ...
            orientation === 'horizontal' &&
              'absolute top-full left-0 min-w-48 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-50 p-2'
          )}
          role='group'
...
const Navigation = React.forwardRef<HTMLElement, NavigationProps>(
  (
    { items, orientation = 'vertical', className, onItemSelect, ariaLabel, collapsible = false },
    ref
  ) => {
    const navigationId = generateId('navigation');
    const [isCollapsed, setIsCollapsed] = React.useState(false);

    const { containerRef, focusedIndex, handleKeyDown, resetFocus } = useListNavigation({
        ...
        >
          ...
          >
            {items.map((item, index) => (
              <NavigationItem key={item.id} item={item} index={index} totalItems={items.length} />
            ))}
          </ul>
