  ...
  ({ className, required, optional, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        {optional && (
          <span className='text-gray-400 ml-1 text-xs font-normal' aria-label='campo opcional'>
            (opcional)
          </span>
        )}
      </label>
...
const FormControl = React.forwardRef<HTMLDivElement, FormControlProps>(
  ({ className, ...props }, ref) => {
    ...
    return <div ref={ref} className={className} {...props} aria-describedby={ariaDescribedBy} />;
  }
);
  ...
  ({ className, children, type = 'error', ...props }, ref) => {
    ...
    return (
      <p
        ref={ref}
        id={`${id}-${type}`}
        className={cn('text-sm flex items-center space-x-1', colorClasses[type], className)}
        role={type === 'error' ? 'alert' : 'status'}
        aria-live='polite'
        {...props}
      >
        <Icon className={cn('w-4 h-4 flex-shrink-0', iconColorClasses[type])} aria-hidden='true' />
        <span>{children}</span>
      </p>
    );
  }
...
const FormInput = React.forwardRef<HTMLInputElement, FormInputProps>(({ name, ...props }, ref) => {
  const { id } = useFormField();

  return <Input ref={ref} id={id} name={name} {...props} />;
});
      ...
      >
        {legend && (
          <legend className='text-sm font-medium text-gray-900 dark:text-gray-100 px-2 -ml-2'>
            {legend}
            {required && (
              <span className='text-red-500 ml-1' aria-label='grupo obrigatório'>
                *
              </span>
            )}
          </legend>
        ...
        {description && (
          <p id={descriptionId} className='text-sm text-gray-600 dark:text-gray-400 -mt-2'>
            {description}
          </p>
        )}
