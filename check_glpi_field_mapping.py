...
def check_field_mapping():
    ...
    try:
        ...
        response = requests.get(search_url, headers=search_headers, params=search_params, timeout=60)
        ...
        if response.status_code in [200, 206]:
            ...
            for i, ticket in enumerate(tickets):
                ...
                # Procurar por datas
                date_fields = [k for k, v in ticket.items() if isinstance(v, str) and '-' in v and ':' in v]
                print(f"   Possíveis datas: {date_fields}")
                
                # Procurar por números pequenos (prioridade, status)
                small_numbers = [k for k, v in ticket.items() if isinstance(v, int) and 0 <= v <= 10]
