...
class GLPIKnowledgeExtractor:
    ...
    def _filter_relevant_tickets(self, tickets: List[Dict]) -> List[Dict]:
        ...
        basic_keywords = [
            ...
            'wifi', 'rede', 'network', 'conexão',
            'software', 'programa', 'aplicativo',
            'instalação', 'instalar', 'instalar',
            'configuração', 'configurar',
            'reset', 'reiniciar', 'restart',
            'atualização', 'update', 'atualizar',
            'licença', 'license', 'ativação',
            'backup', 'cópia', 'restauração',
            'antivírus', 'antivirus', 'segurança',
            'teclado', 'mouse', 'monitor',
            'som', 'áudio', 'audio',
            'câmera', 'webcam', 'microfone'
        ]
        ...
        for ticket in tickets:
            ...
            if any(keyword in content for keyword in basic_keywords):
                ...
                if priority in ['1', '2', '3']:
                    ...
                    self.stats['technicians'][tech] = (
                        self.stats['technicians'].get(tech, 0) + 1
                    )
    ...
    def generate_knowledge_files(self, tickets: List[Dict]) -> None:
        ...
        for ticket in tickets:
            try:
                ...
                # Salvar arquivo
                file_path = (
                    base_path / "tickets_resolvidos" / category / filename
                )
    ...
    def _categorize_ticket(self, ticket: Dict) -> str:
        ...
        if any(keyword in content for keyword in [
            'impressora', 'printer', 'print', 'teclado', 'mouse', 'monitor',
            'computador', 'pc', 'notebook', 'desktop', 'hardware'
        ]):
        ...
        # Software
            ...
            'software', 'programa', 'aplicativo', 'app', 'instalação',
            'instalar', 'licença', 'license', 'atualização', 'update'
        ]):
            return 'software/aplicativos'
        
        # Rede
        elif any(keyword in content for keyword in [
            'wifi', 'rede', 'network', 'conexão', 'conectar', 'internet',
            'vpn', 'roteador', 'router', 'switch'
        ]):
            return 'rede/wifi'
        
        # Sistemas
        elif any(keyword in content for keyword in [
            'email', 'outlook', 'correio', 'senha', 'password', 'login',
            'acesso', 'sistema', 'configuração', 'configurar'
        ]):
...
"""
    ...
    def _generate_tags(self, title: str, description: str, 
...
def main():
    ...
    GLPI_URL = input(
        "URL do GLPI (ex: http://glpi.exemplo.com/glpi): "
    ).strip()
