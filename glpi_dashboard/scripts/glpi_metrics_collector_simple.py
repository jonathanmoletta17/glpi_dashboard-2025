        ...
        try:
            ...
            for tecnico in tecnicos_ativos:
                try:
                    ...
                    print(
                        f"      Erro ao processar tecnico {tecnico.get('nome', 'Desconhecido')}: {e}"
                    )
                    continue
            ...
            for nivel, tecnicos_nivel in ranking_por_nivel.items():
                if tecnicos_nivel:
                    ...
                    for tecnico in tecnicos_nivel[:3]:
                        print(
                            f"      {tecnico['posicao']}o {tecnico['nome']} - {tecnico['tickets_resolvidos']} resolvidos ({tecnico['tickets_total']} total)"
                        )
