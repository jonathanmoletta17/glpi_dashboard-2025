...
class GLPICompleteExtractor:
    ...
    def make_api_request(self, endpoint: str, params: Optional[Dict] = None) -> Optional[List[Dict]]:
    ...
    def extract_with_pagination(self, endpoint: str, description: str) -> List[Dict]:
        ...
        while True:
            ...
            self.logger.info(f"Extraindo {description} - registros {start} a {start + self.page_size - 1}")

            data = self.make_api_request(endpoint, params)

            if not data:
                self.logger.warning(f"Nenhum dado retornado para {description} no range {start}-{start + self.page_size - 1}")
                break
    ...
    def extract_all_users(self) -> List[Dict]:
        ...
        for user in users:
            ...
            for key, value in user.items():
                ...
                if key in ['comment', 'firstname', 'realname', 'name', 'nickname', 'user_dn',
                          'registration_number', 'phone', 'phone2', 'mobile']:
    ...
    def filter_technicians(self, users: List[Dict]) -> List[Dict]:
        ...
        tech_comment_keywords = [
            'cc-se-subadm-dtic', 'dtic', 'tecnico de informatica', 'helpdesk',
            'suporte tecnico', 'administrador de sistema', 'analista de sistemas'
        ]

        # Entidades específicas de TI
        tech_entities = [
            'dtic', 'informatica', 'tecnologia da informacao'
        ]
    ...
    def filter_requesters(self, users: List[Dict]) -> List[Dict]:
        ...
        for user in users:
            ...
            entities_id = str(user.get('entities_id', '')).lower()
            requester_entities = [
                'secretaria', 'gabinete', 'departamento', 'divisao', 'setor',
                'coordenacao', 'assessoria', 'diretoria', 'gerencia'
            ]
    ...
    def print_statistics(self):
        ...
        print("  • data/tickets.csv")
        print("  • data/usuarios.csv")
        print("  • data/tecnicos.csv")
        print("  • data/solicitantes.csv")
        print("="*60)
