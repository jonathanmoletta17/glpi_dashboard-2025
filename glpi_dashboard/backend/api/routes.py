...
Versão refatorada - Sistema de cache unificado
"""
...
def get_glpi_service():
    ...
    try:
        ...
        logger.warning(
            f"Falha ao conectar com GLPI real: {e}. "
            f"Usando serviço mock como fallback"
        )
...
# Cache manual apenas para endpoints que não usam decorator
# (Removido cache duplicado - usando apenas @cache_with_filters)
...
@api_bp.route("/health")
@monitor_api_endpoint("health_check")
@monitor_performance
def health_check():
...
@monitor_api_endpoint("debug_silvio")
@monitor_performance
def debug_silvio():
...
@monitor_api_endpoint("glpi_health_check")
@monitor_performance
...
@monitor_performance
@cache_with_filters(timeout=120)
...
def get_metrics(
    ...
    try:
        ...
        logger.info(
            f"[{correlation_id}] Buscando métricas do GLPI com filtros: "
            f"data={start_date} até {end_date}"
        )
        ...
        if start_date or end_date:
            if filter_type == "modification":
                ...
                metrics_data = (
                    glpi_service.get_dashboard_metrics_with_date_filter(
                        start_date=start_date,
                        end_date=end_date,
                        correlation_id=correlation_id,
                )
        ...
        if (isinstance(metrics_data, dict) and
                metrics_data.get("success") is False):
            return jsonify(metrics_data), 500

        if not metrics_data:
            logger.warning(
                "Não foi possível obter métricas do GLPI, "
        ...
        logger.info(
            f"[{correlation_id}] Métricas obtidas com sucesso em "
        ...
        if (isinstance(metrics_data, dict) and
                "data" in metrics_data):
            metrics_data["correlation_id"] = correlation_id
            metrics_data["cached"] = False

        # Cache gerenciado pelo decorator @cache_with_filters

        return jsonify(metrics_data)

    except Exception as e:
        logger.error(
            f"[{correlation_id}] Erro inesperado ao buscar métricas: {e}",
            exc_info=True
        )
        error_response = ResponseFormatter.format_error_response(
...
def get_filtered_metrics(
    ...
    try:
        ...
        logger.error(
            f"[{correlation_id}] Erro inesperado ao buscar métricas "
            f"filtradas: {e}",
            exc_info=True
        )
        error_response = ResponseFormatter.format_error_response(
...
def get_technicians():
    ...
    try:
        ...
        technician_ids, technician_names = (
            glpi_service._get_all_technician_ids_and_names(
                entity_id=entity_id
            )
        )
        ...
        correlation_id = getattr(locals(), 'correlation_id',
                               api_logger.generate_correlation_id())
        logger.error(f"[{correlation_id}] Erro ao buscar técnicos: {e}", exc_info=True)
        error_response = ResponseFormatter.format_error_response(
...
@monitor_performance
@cache_with_filters(timeout=120)  # REABILITADO - CACHE FUNCIONANDO
@standard_date_validation(support_predefined=True, log_usage=True)
def get_technician_ranking(
    ...
    try:
        ...
        if ranking_data is None:
            ...
            error_response = ResponseFormatter.format_error_response("Não foi possível conectar ao GLPI",
                ["Erro de conexão"])
            return jsonify(error_response), 503
        ...
        logger.error(f"[{correlation_id}] Erro inesperado ao buscar ranking de técnicos: {e}", exc_info=True)
        error_response = ResponseFormatter.format_error_response(
            f"Erro interno do servidor: {str(e)}", [str(e)],
            correlation_id=correlation_id
        )
...
def get_new_tickets(
    ...
    try:
        ...
        error_response = ResponseFormatter.format_error_response(f"Erro interno do servidor: {str(e)}",
            [str(e)])
        return jsonify(error_response), 500
...
@api_bp.route("/status")
@monitor_api_endpoint("get_status")
@monitor_performance
def get_status():
...
@monitor_api_endpoint("debug_field_values")
@monitor_performance
def debug_field_values():
    ...
    try:
        ...
        field_values = glpi_service.debug_field_values(
            correlation_id=correlation_id
        )
...
@monitor_api_endpoint("debug_technician_tickets")
@monitor_performance
def debug_technician_tickets():
    ...
    try:
        ...
        if technician_id:
            ...
            debug_data = glpi_service.debug_technician_tickets(technician_id,
                correlation_id=correlation_id
            )
        else:
            # Debug geral dos primeiros técnicos
            debug_data = glpi_service.debug_technician_tickets_general(limit=limit,
                correlation_id=correlation_id
            )
