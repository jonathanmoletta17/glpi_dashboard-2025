...
from utils.prometheus_metrics import monitor_glpi_request, prometheus_metrics
...
class GLPIService:
    ...
    def __init__(self):
        try:
            ...
            if (
                not hasattr(config_obj, "GLPI_APP_TOKEN")
                or not config_obj.GLPI_APP_TOKEN
            ):
                raise ValueError("GLPI_APP_TOKEN não está configurado")
            if (
                not hasattr(config_obj, "GLPI_USER_TOKEN")
                or not config_obj.GLPI_USER_TOKEN
            ):
            ...
            if not self.glpi_url.startswith(("http://", "https://")):
                raise ValueError(
                    f"GLPI_URL deve começar com http:// ou https://, recebido: {self.glpi_url}"
                )
    ...
    def _is_cache_valid(self, cache_key: str, sub_key: str = None) -> bool:
        ...
        try:
            ...
            if sub_key is not None and (
                not isinstance(sub_key, str) or not sub_key.strip()
            ):
            ...
            if not is_valid:
                self.logger.debug(
                    f"Cache expirado para {cache_key}: idade={current_time - timestamp:.1f}s, TTL={ttl}s"
                )
    ...
    def _set_cache_data(
        ...
        try:
            ...
            if sub_key:
                ...
                self.logger.debug(
                    f"Cache definido para {cache_key}[{sub_key}] com TTL {ttl}s"
                )
            else:
    ...
    def _is_token_expired(self) -> bool:
        ...
        try:
            ...
            if not self.token_created_at or not isinstance(
                self.token_created_at, (int, float)
            ):
                self.logger.debug(
                    "Timestamp de criação do token não existe ou é inválido"
                )
                return True
            ...
            if self.token_created_at > current_time:
                self.logger.warning(
                    "Timestamp do token está no futuro, considerando expirado"
                )
                return True
    ...
    def _ensure_authenticated(self) -> bool:
        ...
        try:
            ...
            if (
                ...
                self.logger.info(
                    "Token de sessão não existe ou é inválido, autenticando..."
                )
    ...
    def _authenticate_with_retry(self) -> bool:
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    self.logger.info(
                        f"Tentativa de autenticação {attempt + 1}/{self.max_retries}"
                    )
                    ...
                    if attempt < self.max_retries - 1:
                        delay = min(
                            self.retry_delay_base**attempt, 30
                    ...
                    self.logger.error(
                        f"Erro de conexão na tentativa {attempt + 1}: {e}"
                    )
                    if attempt < self.max_retries - 1:
                        delay = min(self.retry_delay_base**attempt, 30)
                        time.sleep(delay)
            ...
            self.logger.error(
                f"Falha na autenticação após {self.max_retries} tentativas"
            )
            return False

        except Exception as e:
            self.logger.error(
                f"Erro crítico no processo de autenticação com retry: {e}"
            )
            return False
    ...
    def _perform_authentication(self) -> bool:
        ...
        try:
            ...
            if (
                not self.glpi_url
                or not isinstance(self.glpi_url, str)
                or not self.glpi_url.strip()
            ):
            ...
            if not response_data or "session_token" not in response_data:
                self.logger.error(
                    f"session_token não encontrado na resposta: {response_data}"
                )
                return False
            ...
            self.logger.info(
                f"Autenticação bem-sucedida! Token expira em {self.session_timeout}s"
            )
            return True
    ...
    def get_api_headers(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if not self._ensure_authenticated():
                self.logger.error(
                    "Não foi possível obter headers - falha na autenticação"
                )
                return None
    ...
    def _make_authenticated_request(
        self, method: str, url: str, correlation_id: Optional[str] = None, **kwargs
        ...
        try:
            ...
            if (
                ...
                or method.strip().upper()
                not in ["GET", "POST", "PUT", "DELETE", "PATCH"]
            ):
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    self.logger.debug(
                        f"[DEBUG] Response headers: {dict(response.headers)}"
                    )
                    self.logger.debug(
                        f"[DEBUG] Response text (primeiros 200 chars): {response.text[:200]}"
                    )
                    ...
                    with open("debug_ranking.log", "a", encoding="utf-8") as f:
                        f.write(
                            f"[DEBUG] Requisição {method} {url} - Status: {response.status_code}, OK: {response.ok}\n"
                        )
                        f.write(f"[DEBUG] Response text: {response.text[:500]}\n")

                    # Registrar métricas da requisição GLPI
                    prometheus_metrics.record_glpi_request(
                        endpoint=f"{method} {url.split('/')[-1]}",
                    ...
                    # Log de performance e alertas para requisições lentas
                    if response_time > 5.0:
                        self.logger.warning(
                            f"Requisição lenta detectada: {response_time:.2f}s para {method} {url}"
                        )
                        # Registrar alerta para resposta lenta
                        ...
                        self.logger.debug(
                            f"Requisição completada em {response_time:.2f}s"
                        )
                    ...
                    if response.status_code >= 500:
                        ...
                        self.logger.debug(
                            f"Requisição bem-sucedida: {response.status_code}"
                        )
                    ...
                    self.logger.warning(
                        f"Timeout na requisição (tentativa {attempt + 1}): {e}"
                    )
                    # Incrementar contador de erros Prometheus
                    prometheus_metrics.glpi_errors_total.labels(
                        method=method, error_type="timeout"
                    ).inc()
                    ...
                    # Incrementar contador de erros Prometheus
                    prometheus_metrics.glpi_errors_total.labels(
                        method=method, error_type="connection_error"
                    ).inc()
                    ...
                    self.logger.error(
                        f"Erro na requisição (tentativa {attempt + 1}): {e}"
                    )
                    if attempt < self.max_retries - 1:
                        delay = min(self.retry_delay_base**attempt, 30)
                        time.sleep(delay)
            ...
            self.logger.error(
                f"Erro crítico no método _make_authenticated_request: {e}"
            )
            return None
    ...
    def discover_field_ids(self) -> bool:
        ...
        try:
            ...
            if not hasattr(self, "field_ids") or not isinstance(self.field_ids, dict):
                self.field_ids = {}
                self.logger.warning(
                    "field_ids não inicializado, criando novo dicionário"
                )
            ...
            try:
                ...
                if not response.ok:
                    self.logger.error(
                        f"Falha ao descobrir field IDs: HTTP {response.status_code}"
                    )
                    self._apply_fallback_field_ids()
                    return True
                ...
                try:
                    ...
                    self.logger.error(
                        f"Resposta JSON inválida ao descobrir field IDs: {e}"
                    )
                    self._apply_fallback_field_ids()
                    return True
                ...
                if not isinstance(search_options, dict):
                    self.logger.error(
                        "Formato de resposta inválido para search options"
                    )
                    self._apply_fallback_field_ids()
                    return True
                ...
                for field_id, field_info in search_options.items():
                    try:
                        ...
                        if not fields_found["GROUP"] and any(
                            name.lower() in field_name.lower()
                            for name in tech_group_field_names
                        ):
                        ...
                        # Buscar campo de status
                            ...
                            name.lower() in field_name.lower()
                            for name in status_field_names
                        ):
                        ...
                        # Buscar campo de técnico
                            ...
                            name.lower() in field_name.lower()
                            for name in tech_field_names
                        ):
                            self.field_ids["TECH"] = str(field_id)
                            fields_found["TECH"] = True
                            self.logger.info(
                                f"Campo TECH encontrado: ID {field_id} - {field_name}"
                            )
                ...
                self.logger.info(
                    "Campo DATE_CREATION definido como ID 15 (padrão GLPI)"
                )
                ...
                if missing_fields:
                    self.logger.error(
                        f"Campos críticos ainda ausentes: {missing_fields}"
                    )
                    return False

                # Salvar no cache para evitar descobertas futuras
                self._set_cache_data(
                    "field_ids", self.field_ids.copy(), ttl=1800
                )  # 30 minutos
                self.logger.info(
                ...
                self.logger.error(
                    f"Erro de requisição ao descobrir IDs dos campos: {e}"
                )
                self._apply_fallback_field_ids()
                return True
    ...
    def _apply_fallback_field_ids(self):
        ...
        try:
            ...
            if not hasattr(self, "field_ids"):
                ...
                self.logger.error(
                    f"field_ids tem tipo inválido: {type(self.field_ids)}, recriando"
                )
                self.field_ids = {}
            ...
            for field_name, fallback_id in fallbacks.items():
                try:
                    ...
                    self.logger.error(
                        f"Erro ao processar fallback para campo {field_name}: {e}"
                    )
            ...
            for field in required_fields:
                if field not in self.field_ids or not self.field_ids[field]:
                    self.logger.error(
                        f"Campo crítico {field} ainda não configurado após fallbacks"
                    )
            ...
            try:
                ...
                self.logger.error(
                    f"Falha crítica ao aplicar configuração mínima: {critical_error}"
                )
                raise
    ...
    def _get_technician_name(self, tech_id: str) -> str:
        ...
        try:
            ...
            if not tech_id.isdigit():
                self.logger.debug(
                    f"tech_id não numérico: {tech_id}, retornando nome baseado no ID"
                )
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Resposta JSON inválida para usuário {tech_id}: {e}"
                    )
                    return f"Técnico {tech_id}"
                ...
                if isinstance(user_data, list):
                    if user_data and isinstance(user_data[0], dict):
                        ...
                        self.logger.warning(
                            f"Lista de dados inválida para usuário {tech_id}"
                        )
                        return f"Técnico {tech_id}"
                ...
                try:
                    ...
                    if firstname and lastname:
                        ...
                        self.logger.debug(
                            f"Apenas sobrenome para técnico {tech_id}: {lastname}"
                        )
                        return lastname
                except Exception as e:
                    self.logger.warning(
                        f"Erro ao combinar nomes para usuário {tech_id}: {e}"
                    )

                # Fallback final
                self.logger.warning(
                    f"Nenhum nome válido encontrado para técnico {tech_id}"
                )
                return f"Técnico {tech_id}"

            except requests.exceptions.RequestException as e:
                self.logger.error(
                    f"Erro de requisição ao buscar técnico {tech_id}: {e}"
                )
                return f"Técnico {tech_id}"
            ...
            self.logger.error(
                f"Erro crítico no método _get_technician_name para {tech_id}: {e}"
            )
    ...
    def get_ticket_count_by_hierarchy(
        ...
        try:
            ...
            if not isinstance(level, str) or not level.strip():
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] level inválido: {level}"
                )
                return 0
            ...
            try:
                ...
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro ao converter status_id para int: {e}"
                )
                return 0
            ...
            if not hasattr(self, "glpi_url") or not self.glpi_url:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] GLPI URL não configurada"
                )
                return 0
            ...
            if not self._ensure_authenticated():
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Falha na autenticação"
                )
                return 0
            ...
            if not self.field_ids.get("STATUS"):
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Field ID STATUS não encontrado: {self.field_ids.get('STATUS')}"
                )
                return 0
            ...
            self.logger.info(
                f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Buscando tickets por hierarquia - level: {level}, status: {status_id}"
            )
            ...
            self.logger.error(
                f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro de requisição: {e}"
            )
            return 0
        except Exception as e:
            self.logger.error(
                f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro inesperado: {e}"
            )
            return 0
            ...
            try:
                ...
                try:
                    ...
                    if (
                        ...
                        and "content-range" in response_data
                    ):
                    ...
                    if (
                        isinstance(response_data, dict)
                        and "totalcount" in response_data
                    ):
    ...
    def _get_metrics_by_level_internal(
        ...
        try:
            ...
            if not hasattr(self, "service_levels") or not isinstance(
                self.service_levels, dict
            ):
                self.logger.error(
            ...
            if not self.status_map:
                self.logger.warning(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] status_map está vazio"
                )
                return {}
    ...
    def _get_aggregated_ticket_counts(
        ...
        try:
            ...
            for i, level in enumerate(levels):
                if i == 0:
                    search_params[f"criteria[{criteria_index}][field]"] = "8"
                    search_params[f"criteria[{criteria_index}][searchtype]"] = "equals"
                    search_params[f"criteria[{criteria_index}][value]"] = level
                else:
                    criteria_index += 1
            ...
            try:
                ...
                while True:
                    ...
                    for ticket in page_data["data"]:
                        try:
                            ticket_level = ticket.get(
                                "8", ""
                            ).strip()  # Campo hierarquia
                            ticket_status_id = int(ticket.get("12", 0))  # Campo status

                            if ticket_level in result:
                            ...
                            self.logger.debug(
                                f"{correlation_log}Erro ao processar ticket: {e}"
                            )
                            continue
                ...
                total_tickets = sum(
                    sum(level_data.values()) for level_data in result.values()
                )
                self.logger.info(
    ...
    def _get_aggregated_ticket_counts_fallback(
        ...
        correlation_log = f"[{correlation_id}] " if correlation_id else ""
        self.logger.info(
            f"{correlation_log}[FALLBACK] Usando método individual para contagens"
        )
        result = {level: {} for level in levels}
    ...
    def _get_metrics_by_level_internal_hierarchy(
        ...
        try:
            ...
            metrics = self._get_aggregated_ticket_counts(
                hierarchy_levels, status_ids, start_date, end_date, correlation_id
            )
    ...
    def get_dashboard_metrics(
        ...
        try:
            ...
            try:
                ...
                general_pendentes = (
                    general_totals.get("Pendente", 0) if general_totals else 0
                )
                general_progresso = (
                    (
                ...
                general_resolvidos = (
                    (
                        general_totals.get("Solucionado", 0)
                ...
                return ResponseFormatter.format_error_response(
                    "Erro ao calcular totais", [str(e)], correlation_id=correlation_id
                )
                    ...
                    for level_name, level_data in raw_metrics.items():
                        try:
                            ...
                            if level_key in level_metrics:
                                ...
                                progresso_atribuido = level_data.get(
                                    "Processando (atribuído)", 0
                                )
                                progresso_planejado = level_data.get(
                                ...
                                for name, value in [
                                    ("novos", novos),
                                    ("progresso_atribuido", progresso_atribuido),
                                ...
                                level_metrics[level_key]["pendentes"] = max(
                                    0,
                                    int(pendentes)
                                    if isinstance(pendentes, (int, float))
                                    else 0,
                                )
                                level_metrics[level_key]["resolvidos"] = max(
                                    ...
                                    + (
                                        int(fechado)
                                        if isinstance(fechado, (int, float))
                                        else 0
                                    ),
                                )
                            else:
            ...
            try:
                ...
                self.logger.debug(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Resultado salvo no cache"
                )
            except Exception as e:
                self.logger.warning(
            ...
            return ResponseFormatter.format_error_response(
                f"Erro interno: {str(e)}", [str(e)], correlation_id=correlation_id
            )
    ...
    def _get_general_totals_internal(
        ...
        try:
            ...
            if not hasattr(self, "field_ids") or not self.field_ids:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] field_ids não configurado"
                )
                return {}
        ...
        for status_name, status_id in self.status_map.items():
            try:
                ...
                criterios_data = DateValidator.construir_criterios_filtro_data(
                    start_date, end_date
                )
        ...
        try:
            ...
            if start_date:
                if not isinstance(start_date, str):
                    self.logger.error(
                        f"start_date deve ser string, recebido: {type(start_date)}"
                    )
                    return None
                try:
                    ...
                    self.logger.error(
                        f"Formato inválido para start_date '{start_date}': {e}"
                    )
                    return None
            ...
            if end_date:
                if not isinstance(end_date, str):
                    self.logger.error(
                        f"end_date deve ser string, recebido: {type(end_date)}"
                    )
                    return None
                try:
                    ...
                    self.logger.error(
                        f"Formato inválido para end_date '{end_date}': {e}"
                    )
                    return None
            ...
            try:
                if self._is_cache_valid("dashboard_metrics_filtered", cache_key):
                    cached_data = self._get_cache_data(
                        "dashboard_metrics_filtered", cache_key
                    )
                    if cached_data:
                        self.logger.info(
            ...
            try:
                ...
                if not isinstance(general_totals, dict):
                    ...
                    return None
                self.logger.info(
                    f"Totais gerais obtidos com filtro de data: {general_totals}"
                )
            except Exception as e:
            ...
            try:
                ...
                if not isinstance(raw_metrics, dict):
                    self.logger.error(
                        f"raw_metrics deve ser dict, recebido: {type(raw_metrics)}"
                    )
                    return None
                self.logger.info(
                    f"Métricas por nível obtidas: {len(raw_metrics)} níveis"
                )
            except Exception as e:
            ...
            try:
                ...
                for level_name, level_data in raw_metrics.items():
                    try:
                        ...
                        processando_atribuido = level_data.get(
                            "Processando (atribuído)", 0
                        )
                        processando_planejado = level_data.get(
                            "Processando (planejado)", 0
                        )
                        ...
                        level_metrics[level_key]["progresso"] = int(
                            processando_atribuido
                        ) + int(processando_planejado)
                        ...
                        if not isinstance(fechado, (int, float)):
                            ...
                            fechado = 0
                        level_metrics[level_key]["resolvidos"] = int(solucionado) + int(
                            fechado
                        )
            ...
            try:
                ...
                general_progresso_atribuido = general_totals.get(
                    "Processando (atribuído)", 0
                )
                general_progresso_planejado = general_totals.get(
                    "Processando (planejado)", 0
                )
                ...
                for name, value in [
                    ...
                    if not isinstance(value, (int, float)):
                        self.logger.warning(
                            f"Valor inválido para '{name}': {value}, usando 0"
                        )
                        if name == "Novo":
                            general_novos = 0
                        elif name == "Pendente":
    ...
    def get_technician_ranking(self, limit: int = None) -> list:
        ...
        try:
            ...
            if not hasattr(self, "glpi_url") or not self.glpi_url:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] glpi_url não configurado"
                )
                return []
            ...
            try:
                ...
                if cached_data is not None:
                    self.logger.info(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Retornando ranking de técnicos do cache"
                    )
            ...
            try:
                ...
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro na autenticação: {e}"
                )
                return []
            ...
            try:
                if ranking:
                    ...
                    self.logger.info(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Dados armazenados no cache"
                    )
            except Exception as e:
                self.logger.warning(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro ao salvar no cache: {e}"
    ...
    def _get_technician_ranking_knowledge_base(self) -> list:
        ...
        try:
            ...
            self.logger.info(
                f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Iniciando consulta otimizada de ranking de técnicos"
            )
            ...
            self.logger.info(
                f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Buscando usuários com perfil ID 6"
            )
            ...
            try:
                ...
                if not response:
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Falha ao buscar usuários com perfil de técnico"
                    )
                    return []

                if not response.ok:
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro HTTP ao buscar Profile_User: {response.status_code}"
                    )
                    return []

            except requests.exceptions.Timeout:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Timeout ao buscar Profile_User"
                )
                return []
            except requests.exceptions.ConnectionError:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro de conexão ao buscar Profile_User"
                )
                return []
            except requests.exceptions.RequestException as e:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro de requisição ao buscar Profile_User: {e}"
                )
                return []
            ...
            try:
                ...
                if not isinstance(profile_result, dict):
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Resposta inválida da busca de Profile_User: {type(profile_result)}"
                    )
                    return []

                self.logger.info(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Resposta da busca de Profile_User recebida"
                )
            except ValueError as e:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro ao decodificar JSON da resposta Profile_User: {e}"
                )
                return []
            ...
            # Log de observabilidade: contagem de técnicos encontrados
            timestamp = datetime.datetime.now(tz=datetime.timezone.utc).isoformat()
            self.logger.info(
                f"[{timestamp}] GLPI Technician Count Result - "
                f"profile_users_found: {total_count}, "
                f"endpoint: Profile_User, profile_id: 6"
            )
            ...
            # Buscar usuários ativos (GLPI não possui campo is_deleted real, apenas is_active)
            user_params = {
                "range": "0-999",
            ...
            # Log de observabilidade: parâmetros GLPI para busca de usuários ativos
            timestamp = datetime.datetime.now(tz=datetime.timezone.utc).isoformat()
            self.logger.info(
                f"[{timestamp}] GLPI Active Users Query Parameters - "
                f"is_active: 1, range: 0-999, "
                f"display_fields: [2=id, 1=username, 9=realname, 34=firstname], "
                f"endpoint: User"
            )
            ...
            try:
                ...
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Timeout na busca de usuários ativos"
                )
                return []
            except requests.exceptions.ConnectionError:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro de conexão na busca de usuários ativos"
                )
                return []
            except requests.exceptions.RequestException as e:
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro na requisição de usuários ativos: {e}"
                )
                return []
            ...
            try:
                ...
                self.logger.error(
                    f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro ao decodificar JSON da resposta de usuários: {e}"
                )
                return []
            ...
            with open("debug_technician_ranking.log", "a", encoding="utf-8") as f:
                f.write(
                    f"{datetime.datetime.now()} - Resultado da busca de usuários: totalcount={user_result.get('totalcount', 0) if isinstance(user_result, dict) else 'N/A'}\n"
                )
                f.flush()
            ...
            for username in active_tech_usernames:
                ...
                try:
                    ...
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Timeout ao buscar dados do técnico {user_id}"
                    )
                    continue
                except requests.exceptions.ConnectionError:
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro de conexão ao buscar dados do técnico {user_id}"
                    )
                    continue
                except requests.exceptions.RequestException as e:
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro na requisição do técnico {user_id}: {e}"
                    )
                    continue
                ...
                try:
                    ...
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Erro ao decodificar JSON do técnico {user_id}: {e}"
                    )
                    with open(
                        "debug_technician_ranking.log", "a", encoding="utf-8"
                    ) as f:
                ...
                if not user_data or not isinstance(user_data, dict):
                    self.logger.error(
                        f"[{datetime.datetime.now(tz=datetime.timezone.utc).isoformat()}] Dados inválidos para técnico {user_id}: {type(user_data)}"
                    )
                    with open(
                        "debug_technician_ranking.log", "a", encoding="utf-8"
                    ) as f:
                        f.write(
                            f"{datetime.datetime.now()} - ERRO: Dados vazios ou inválidos para técnico {user_id}\n"
                        )
                        f.flush()
                    continue
                ...
                if user_data:
                    ...
                    try:
                        ...
                        # Obter dados detalhados dos tickets do técnico
                        try:
                            ticket_data = self._get_technician_ticket_details(
                                int(user_id), tech_field_id
                            )

                            if ticket_data is not None and isinstance(
                                ticket_data, dict
                            ):
                                ranking.append(
                                    {
                                        ...
                                        "total_tickets": ticket_data.get(
                                            "total_tickets", 0
                                        ),
                                        "resolved_tickets": ticket_data.get(
                                            "resolved_tickets", 0
                                        ),
                                        "pending_tickets": ticket_data.get(
                                            "pending_tickets", 0
                                        ),
                                        "avg_resolution_time": ticket_data.get(
                                ...
                                self.logger.info(
                                    f"Técnico {display_name} (ID: {user_id}): {ticket_data.get('total_tickets', 0)} tickets"
                                )
                            else:
                                self.logger.warning(
                                    f"Dados de tickets inválidos para técnico {display_name} (ID: {user_id}): {ticket_data}"
                                )
                        except Exception as count_error:
                            self.logger.error(
                                f"Erro ao obter dados de tickets para técnico {display_name} (ID: {user_id}): {count_error}"
                            )
                            continue
            ...
            try:
                ...
                ranking.sort(key=lambda x: x.get("total_tickets", 0), reverse=True)
                ...
                for idx, item in enumerate(ranking):
                    try:
                        ...
                        level = self._get_technician_level(
                            user_id, item["total_tickets"], ranking
                        )
                        ...
                        self.logger.info(
                            f"Técnico {item['name']} (Rank {idx + 1}): {item['total_tickets']} tickets - Nível: {level}"
                        )
    ...
    def _get_technician_level(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"Erro de conexão na busca de grupos para usuário {user_id}"
                )
            ...
            try:
                ...
                self.logger.error(
                    f"Erro de conexão na busca de dados do usuário {user_id}"
                )
            except requests.exceptions.RequestException as req_error:
                self.logger.error(
                    f"Erro na requisição de dados do usuário {user_id}: {req_error}"
                )
    ...
    def _get_technician_level_by_name(self, tech_name: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Erro ao determinar nível do técnico por nome {tech_name}: {e}"
            )
            return "N1"  # Nível padrão em caso de erro
    ...
    def _get_technician_ranking_fallback(self) -> list:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"Erro ao buscar técnicos ativos no fallback: {techs_error}"
                )
                return []
            ...
            try:
                ...
                if not tech_field_id:
                    self.logger.error(
                        "ID do campo de técnico não encontrado no fallback"
                    )
                    return []
            ...
            for tech_id, tech_name in active_techs:
                try:
                    if not tech_name or not tech_name.strip():
                        self.logger.warning(
                            f"Nome de técnico inválido para ID {tech_id}"
                        )
                        continue
            ...
            try:
                ...
                self.logger.error(
                    f"Erro ao processar ranking final no fallback: {sort_error}"
                )
                return []
    ...
    def _list_active_technicians_fallback(self) -> list:
        ...
        try:
            ...
            self.logger.warning(
                f"Erro ao verificar cache de técnicos ativos: {cache_error}"
            )
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "Iniciando busca paginada de usuários com perfil de técnico"
                )
                ...
                while True:
                    ...
                    if (
                        not page_data
                        or not isinstance(page_data, list)
                        or not page_data
                    ):
                        self.logger.info(
                ...
                if not profile_users:
                    self.logger.warning(
                        "Nenhum usuário encontrado com perfil de técnico"
                    )
                    return []
                ...
                self.logger.error(
                    "Erro de conexão na busca de usuários com perfil de técnico"
                )
                return []
            except requests.exceptions.RequestException as req_error:
                self.logger.error(
            ...
            if not tech_user_ids:
                self.logger.warning(
                    "Nenhum usuário válido encontrado com perfil de técnico"
                )
                return []
            ...
            for i in range(0, len(tech_user_ids), batch_size):
                ...
                for user_id in batch_ids:
                    try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if isinstance(is_active, str):
                                        is_active = (
                                            int(is_active) if is_active.isdigit() else 0
                                        )
                                    if isinstance(is_deleted, str):
                                        is_deleted = (
                                            int(is_deleted)
                                            if is_deleted.isdigit()
                                            else 1
                                        )
                                    ...
                                    if is_active == 1 and is_deleted == 0:
                                        ...
                                        try:
                                            ...
                                            if display_name and isinstance(
                                                display_name, str
                                            ):
                        ...
                        self.logger.error(
                            f"Erro de conexão na busca do usuário {user_id}"
                        )
                        continue
                    except requests.exceptions.RequestException as req_error:
                        self.logger.error(
                            f"Erro na requisição do usuário {user_id}: {req_error}"
                        )
                        continue
                    except Exception as user_error:
                        self.logger.error(
            ...
            try:
                ...
                self.logger.warning(
                    f"Erro ao armazenar técnicos no cache: {cache_error}"
                )
    ...
    def _count_tickets_by_technician_optimized(
        ...
        try:
            ...
            if not self.glpi_url:
                self.logger.error(
                    "URL do GLPI não configurada para contagem de tickets"
                )
                return None
            ...
            self.logger.info(
                f"Contando tickets para técnico {tech_id} com field {tech_field}"
            )
            ...
            try:
                ...
                self.logger.error(
                    f"Timeout na contagem de tickets do técnico {tech_id}"
                )
                return None
            except requests.exceptions.ConnectionError:
                self.logger.error(
                    f"Erro de conexão na contagem de tickets do técnico {tech_id}"
                )
                return None
            except requests.exceptions.RequestException as req_error:
                self.logger.error(
                ...
                if "Content-Range" in response.headers:
                    ...
                    if "/" in content_range:
                        ...
                        if total_str.isdigit():
                            ...
                            if total >= 0:
                                self.logger.info(
                                    f"Técnico {tech_id}: {total} tickets encontrados"
                                )
                                return total
                            else:
                                self.logger.warning(
                    ...
                    self.logger.warning(
                        f"Content-Range não encontrado para técnico {tech_id}"
                    )
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Erro ao processar resposta JSON: {json_error}"
                        )
                    return 0
    ...
    def _get_technician_ticket_details(
        self, tech_id: int, tech_field_id: str
    ) -> Optional[dict]:
        ...
        try:
            ...
            result = self._get_technician_ticket_details_optimized(
                [tech_id], tech_field_id
            )
            return result.get(
                str(tech_id),
    ...
    def _get_technician_ticket_details_legacy(
        self, tech_id: int, tech_field_id: str
    ) -> Optional[dict]:
        ...
        try:
            ...
            response = self._make_authenticated_request(
                "GET", f"{self.glpi_url}/search/Ticket", params=params_total, timeout=30
            )
            if response and response.ok and "Content-Range" in response.headers:
                content_range = response.headers["Content-Range"]
                if "/" in content_range:
                    total_str = content_range.split("/")[-1].strip()
                    if total_str.isdigit():
                        total_tickets = int(total_str)

            # 2. Contar tickets resolvidos (status 5 = Solved)
            params_solved = {
                "is_deleted": 0,
                "criteria[0][field]": tech_field,
                "criteria[0][searchtype]": "equals",
                ...
                "criteria[1][searchtype]": "equals",
                "criteria[1][value]": "5",  # Status Solved
                "range": "0-0",
            }
            ...
            response = self._make_authenticated_request(
                "GET",
                f"{self.glpi_url}/search/Ticket",
                params=params_solved,
                timeout=30,
            )
            if response and response.ok and "Content-Range" in response.headers:
                content_range = response.headers["Content-Range"]
                if "/" in content_range:
                    solved_str = content_range.split("/")[-1].strip()
                    if solved_str.isdigit():
                        solved_tickets = int(solved_str)

            # 2b. Contar tickets fechados (status 6 = Closed)
            params_closed = {
                "is_deleted": 0,
                "criteria[0][field]": tech_field,
                "criteria[0][searchtype]": "equals",
                ...
                "criteria[1][field]": "12",  # Campo status
                "criteria[1][searchtype]": "equals",
                "criteria[1][value]": "6",  # Status Closed
                "range": "0-0",
            }
            ...
            if response and response.ok and "Content-Range" in response.headers:
                content_range = response.headers["Content-Range"]
                if "/" in content_range:
                    closed_str = content_range.split("/")[-1].strip()
                    if closed_str.isdigit():
                        closed_tickets = int(closed_str)

            # Total de tickets resolvidos
            resolved_tickets = solved_tickets + closed_tickets

            # 3. Contar tickets pendentes (status 1-4: New, Assigned, Planned, Waiting)
            pending_tickets = 0
            pending_statuses = ["1", "2", "3", "4"]  # New, Assigned, Planned, Waiting
            ...
            for status in pending_statuses:
                ...
                response = self._make_authenticated_request(
                    "GET",
                    f"{self.glpi_url}/search/Ticket",
                    params=params_pending,
                    timeout=30,
                )
                if response and response.ok and "Content-Range" in response.headers:
                    content_range = response.headers["Content-Range"]
                    if "/" in content_range:
                        status_str = content_range.split("/")[-1].strip()
                        if status_str.isdigit():
                            pending_tickets += int(status_str)

            # 4. Calcular tempo médio de resolução (simplificado - usar 0 por enquanto)
            # Para implementação completa, seria necessário buscar tickets resolvidos
            # e calcular a diferença entre date e solvedate
            avg_resolution_time = 0.0
    ...
    def _get_technician_ticket_details_optimized(
        ...
        try:
            ...
            self.logger.info(
                f"Dados de tickets obtidos para {len(result)} técnicos em uma única consulta"
            )
            return result
    ...
    def _get_all_technician_ids_and_names(
        ...
        try:
            timestamp = datetime.datetime.now(tz=datetime.timezone.utc).isoformat()
            self.logger.info(
                f"[{timestamp}] Iniciando busca de técnicos com método alternativo"
            )

            print(
                f"[DEBUG] _get_all_technician_ids_and_names INICIADO com entity_id: {entity_id}"
            )
            with open("debug_ranking.log", "a", encoding="utf-8") as f:
                f.write(
                    f"[DEBUG] _get_all_technician_ids_and_names INICIADO com entity_id: {entity_id}\n"
            ...
            # MÉTODO ALTERNATIVO: Buscar técnicos através de tickets atribuídos
            # Isso contorna o problema da tabela Profile_User vazia

            # Buscar tickets dos últimos 90 dias para identificar técnicos ativos
            ...
            # Adicionar filtro por entidade se especificado
            if entity_id is not None:
                ticket_params["criteria[1][field]"] = "80"  # entities_id
                ticket_params["criteria[1][searchtype]"] = "equals"
                ticket_params["criteria[1][value]"] = str(entity_id)
                ticket_params["criteria[1][link]"] = "AND"

            self.logger.info(
                f"[{timestamp}] Buscando técnicos via tickets atribuídos - "
                f"período: {start_date.strftime('%Y-%m-%d')} a {end_date.strftime('%Y-%m-%d')}, "
                f"entities_id: {entity_id or 'ALL'}"
            )

            # Buscar tickets com técnicos atribuídos
            response = self._make_authenticated_request(
                "GET", f"{self.glpi_url}/search/Ticket", params=ticket_params
            )

            if not response or not response.ok:
                self.logger.error(
                    f"[{timestamp}] Falha ao buscar tickets - "
            ...
            print(f"[DEBUG] Tickets response status: {response.status_code}")
            print(f"[DEBUG] Tickets data count: {len(ticket_data)}")
            with open("debug_ranking.log", "a", encoding="utf-8") as f:
                f.write(f"[DEBUG] Tickets response status: {response.status_code}\n")
                f.write(f"[DEBUG] Tickets data count: {len(ticket_data)}\n")
                f.flush()

            if not ticket_data:
                print(f"[DEBUG] PROBLEMA: Nenhum ticket encontrado no período")
                self.logger.warning(
                    f"[{timestamp}] Nenhum ticket encontrado no período"
                )
                return [], {}

            # Extrair IDs únicos de técnicos dos tickets
            tech_ids_set = set()
            for ticket in ticket_data:
                if isinstance(ticket, dict) and "5" in ticket:
                    tech_id = self._parse_technician_id(ticket["5"])
                    if tech_id and tech_id != "0":
                        tech_ids_set.add(tech_id)

            print(f"[DEBUG] Técnicos únicos encontrados: {len(tech_ids_set)}")
            with open("debug_ranking.log", "a", encoding="utf-8") as f:
                f.write(
                    f"[DEBUG] Técnicos únicos encontrados: {len(tech_ids_set)} - {list(tech_ids_set)[:5]}\n"
                )
                f.flush()

            if not tech_ids_set:
                self.logger.warning(
                    f"[{timestamp}] Nenhum técnico encontrado nos tickets"
                )
                return [], {}

            # Buscar dados dos usuários técnicos encontrados
            tech_ids = []
            tech_names = {}

            # Processar técnicos em lotes
            tech_ids_list = list(tech_ids_set)
            batch_size = 50

            for i in range(0, len(tech_ids_list), batch_size):
                batch = tech_ids_list[i : i + batch_size]

                # Construir critérios OR para buscar múltiplos usuários por ID
                user_params = {
                    "range": "0-999",
                    "forcedisplay[0]": "2",  # ID
                ...
                # Adicionar critérios OR para cada usuário no lote (buscar por ID)
                for idx, user_id in enumerate(batch):
                    user_params[f"criteria[{idx}][field]"] = "2"  # ID
                    user_params[f"criteria[{idx}][searchtype]"] = "equals"
                    user_params[f"criteria[{idx}][value]"] = str(user_id)
                    if idx > 0:
                        user_params[f"criteria[{idx}][link]"] = "OR"

                with open("debug_ranking.log", "a", encoding="utf-8") as f:
                    f.write(f"[DEBUG] Buscando lote de {len(batch)} usuários\n")

                # Fazer a busca via search API
                user_response = self._make_authenticated_request(
                    "GET", f"{self.glpi_url}/search/User", params=user_params
                )
                ...
                with open("debug_ranking.log", "a", encoding="utf-8") as f:
                    f.write(
                        f"[DEBUG] User search response status: {user_response.status_code if user_response else 'None'}\n"
                    )
                ...
                if user_response and user_response.status_code in [200, 206]:
                    try:
                        ...
                        with open("debug_ranking.log", "a", encoding="utf-8") as f:
                            f.write(
                                f"[DEBUG] Encontrados {len(user_data)} usuários no lote\n"
                            )
                        ...
                        for user in user_data:
                            if isinstance(user, dict):
                                ...
                                if is_active == "1" and (
                                    ...
                                    tech_ids.append(user_id)
                                    # Construir nome completo
                                    full_name = f"{firstname} {realname}".strip()
                                    if not full_name:
                                        full_name = username
                                    tech_names[user_id] = full_name

                                    with open(
                                        "debug_ranking.log", "a", encoding="utf-8"
                                    ) as f:
                                        f.write(
                                            f"[DEBUG] Técnico ativo encontrado: {user_id} - {full_name}\n"
                                        )
                                else:
                                    with open(
                                        "debug_ranking.log", "a", encoding="utf-8"
                                        ...
                                        f.write(
                                            f"[DEBUG] Técnico inativo/deletado: {user_id} - ativo:{is_active}, deletado:{is_deleted}\n"
                                        )

                    except Exception as e:
                        with open("debug_ranking.log", "a", encoding="utf-8") as f:
                            f.write(
                                f"[DEBUG] Erro ao processar resposta da busca de usuários: {e}\n"
                            )
                        self.logger.error(
                            f"Erro ao processar resposta da busca de usuários: {e}"
                        )
                else:
                    with open("debug_ranking.log", "a", encoding="utf-8") as f:
                        f.write(f"[DEBUG] Falha na busca de usuários no lote\n")
                    self.logger.warning("Falha na busca de usuários no lote")

            with open("debug_ranking.log", "a", encoding="utf-8") as f:
                f.write(f"[DEBUG] tech_ids finais: {len(tech_ids)} - {tech_ids[:5]}\n")
    ...
    def get_technicians_by_assignments(self, days_back=30, min_tickets=3):
        ...
        try:
            ...
            if not response or not response.ok:
                self.logger.error("Falha ao buscar tickets com atribuições")
                return {}

            data = response.json()
            if not data or not data.get("data"):
                self.logger.warning("Nenhum ticket encontrado no período")
                return {}

            # Contar atribuições por técnico usando parsing corrigido
            tech_counts = {}
            ...
            for ticket in data["data"]:
                ...
                tech_id = self._parse_technician_id(tech_field)
            ...
            self.logger.error(f"Erro ao identificar técnicos por atribuições: {e}")
            return {}

    def _get_all_tickets_grouped_by_technician(
        self, technician_ids: List[str], start_date: str = None, end_date: str = None
    ) -> Dict[str, int]:
        ...
        try:
            ...
            self.logger.info(
                f"[OTIMIZAÇÃO] Iniciando busca otimizada em lote para {len(technician_ids)} técnicos"
            )
            if start_date and end_date:
                # Com filtros de data - usar query otimizada em lote
                self.logger.info(
                    f"[OTIMIZAÇÃO] Usando busca em lote COM filtro de data: {start_date} a {end_date}"
                )
            ...
            self.logger.info(
                f"[OTIMIZAÇÃO] Busca otimizada concluída: {sum(ticket_counts.values())} tickets total"
            )
            return ticket_counts

        except Exception as e:
            self.logger.error(
                f"[FALLBACK] Erro ao buscar tickets agrupados por técnico: {e}"
            )
            import traceback

            self.logger.error(
                f"[FALLBACK] Traceback completo: {traceback.format_exc()}"
            )
    ...
    def _get_tickets_batch_with_date_filter(
        ...
        try:
            with open("debug_ranking.log", "a", encoding="utf-8") as f:
                f.write(f"[DEBUG] _get_tickets_batch_with_date_filter INICIADO\n")
                f.write(
                    f"[DEBUG] technician_ids count: {len(technician_ids) if technician_ids else 0}\n"
                )
                f.write(f"[DEBUG] start_date: {start_date}, end_date: {end_date}\n")
                f.write(f"[DEBUG] tech_field_id: {tech_field_id}\n")
                f.flush()

            if not technician_ids:
                print(
                    f"[DEBUG] _get_tickets_batch_with_date_filter: technician_ids vazio"
                )
                with open("debug_ranking.log", "a", encoding="utf-8") as f:
                    f.write(f"[DEBUG] PROBLEMA: technician_ids vazio\n")
                    f.flush()
                return {}
            ...
            search_params = {
                "is_deleted": 0,
                "forcedisplay[0]": tech_field_id,  # Campo do técnico descoberto dinamicamente
                "forcedisplay[1]": "2",  # id
            }
            ...
            print(
                f"[DEBUG] Processando {len(technician_ids)} técnicos em lotes otimizados de {batch_size}"
            )
                    ...
                    batch_params.update(
                        {
                            ...
                            f"criteria[{criteria_index}][field]": tech_field_id,  # Campo do técnico descoberto dinamicamente
    ...
    def _fetch_all_pages_robust(
        ...
        try:
            ...
            page_size = 1000  # Tamanho da página para paginação
            start_index = 0
            max_retries = 3
            total_processed = 0
            ...
            while True:
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        # Extrair informações de paginação do cabeçalho Content-Range
                        content_range = response.headers.get("Content-Range", "")
                        if content_range:
                            try:
                                # Formato: "start-end/total" ou "start-end/total"
                                range_part, total_str = content_range.split("/")
                                total_items = int(total_str)
                                start_range, end_range = map(int, range_part.split("-"))

                                self.logger.debug(
                                    f"Página {start_index}-{end_index}: {end_range - start_range + 1} itens de {total_items} total"
                                )
                            except (ValueError, IndexError) as e:
                                self.logger.warning(
                ...
                # Contar tickets por técnico nesta página
                for ticket in page_data["data"]:
                ...
                total_processed += page_items
                self.logger.debug(
                    f"Processados {page_items} tickets na página {start_index}-{end_index}. Total: {total_processed}"
                )
                ...
                if start_index > 100000:  # Máximo de 100k tickets
                    self.logger.warning(
            ...
            self.logger.info(
                f"Paginação robusta concluída: {sum(ticket_counts.values())} tickets encontrados para {len(tech_ids)} técnicos"
            )
            return ticket_counts

        except Exception as e:
            self.logger.error(f"Erro na paginação robusta: {e}")
    ...
    def _process_ticket_batch(
        ...
        try:
            ...
            if not response or not response.ok:
                self.logger.error(
                    f"Falha na requisição do batch: {response.status_code if response else 'No response'}"
                )
                return ticket_counts
            ...
            print(
                f"[DEBUG] Batch processado em {elapsed_time:.2f}s: {sum(ticket_counts.values())} tickets encontrados"
            )
            self.logger.info(
                f"Batch processado: {sum(ticket_counts.values())} tickets encontrados"
            )
            return ticket_counts
            ...
            for tech_id in technician_ids:
                try:
                    if start_date and end_date:
                        ...
                        count = self._count_tickets_with_date_filter(
                    ...
                    self.logger.error(
                        f"Erro ao contar tickets para técnico {tech_id}: {e}"
                    )
    ...
    def get_technician_ranking_with_filters(
        ...
        print(
            f"[DEBUG] get_technician_ranking_with_filters CHAMADO - start_date: {start_date}, end_date: {end_date}"
        )
        ...
        try:
            ...
            print(
                f"[DEBUG] [{correlation_id}] Buscando técnicos ativos com entity_id: {entity_id}"
            )
            technician_ids, technician_names = self._get_all_technician_ids_and_names(
                entity_id=entity_id
            )
            print(
                f"[DEBUG] [{correlation_id}] Encontrados {len(technician_ids) if technician_ids else 0} técnicos"
            )
            print(
                f"[DEBUG] [{correlation_id}] Técnicos encontrados: {technician_ids[:5] if technician_ids else 'Nenhum'}"
            )
            ...
            if not technician_ids:
                ...
                self.logger.warning(
                    f"[{correlation_id}] Nenhum técnico ativo encontrado"
                )
                return []
            ...
            if start_date and end_date:
                ...
                self.logger.info(
                    f"[{correlation_id}] Usando método sem filtros de data"
                )
                ticket_counts = self._get_tickets_batch_without_date_filter(
                    technician_ids, tech_field_id
                )
            ...
            for tech_id in technician_ids:
                try:
                    ...
                    try:
                        ...
                        tech_level = self._get_technician_level(
                            tech_id_int, ticket_count
                        )
                        ...
                        self.logger.debug(
                            f"Nível determinado para {tech_name}: {tech_level}"
                        )
            ...
            obs_logger.log_pipeline_step(
                ...
                "pre_sorting",
                {
                    ...
                    "zero_totals": sum(
                        1 for tech in ranking if tech.get("total", 0) == 0
                    ),
                    "max_total": max([tech.get("total", 0) for tech in ranking])
                    if ranking
                    else 0,
                },
            )
    ...
    def get_new_tickets_with_filters(
        ...
        try:
            ...
            if start_date:
                try:
                    ...
                    self.logger.warning(
                        f"Formato de data de início inválido: {start_date}"
                    )
                    start_date = None
        ...
        if not self.discover_field_ids():
            self.logger.warning(
                "Falha ao descobrir field_ids para buscar tickets novos"
            )
            return []
        ...
        try:
            ...
            if isinstance(data, dict) and "data" in data and data["data"]:
                for ticket_data in data["data"]:
                    try:
                        ...
                        if requester_id:
                            try:
                                requester_name = self._get_user_name_by_id(
                                    str(requester_id)
                                )
                            except Exception:
            ...
            self.logger.info(
                f"Encontrados {len(tickets)} tickets novos com filtros aplicados"
            )
            ...
            self.logger.error(
                f"Erro inesperado ao buscar tickets novos com filtros: {e}"
            )
            return []
    ...
    def _count_tickets_with_date_filter(
        ...
        try:
            ...
            if not self.field_ids:
                if not self.discover_field_ids():
                    self.logger.warning(
                        "Falha ao descobrir field_ids, usando fallbacks"
                    )
                    return 0
            ...
            if start_date:
                start_date_full = (
                    f"{start_date} 00:00:00" if len(start_date) == 10 else start_date
                )
            ...
            self.logger.error(
                f"Erro de conexão ao contar tickets do técnico {tech_id_str}"
            )
            return 0
        except Exception as e:
    ...
    def get_dashboard_metrics_with_modification_date_filter(
        self, start_date: str, end_date: str, correlation_id: Optional[str] = None
    ) -> dict:
        ...
        try:
            ...
            timestamp = datetime.datetime.now(tz=datetime.timezone.utc).isoformat()
            self.logger.info(
                f"[{timestamp}] Obtendo métricas com filtro de modificação: {start_date} a {end_date}"
            )

            # Obter métricas por data de modificação
            totals = self._get_general_metrics_by_modification_date(
                start_date, end_date
            )
            metrics_by_level = self._get_metrics_by_level_by_modification_date(
                start_date, end_date
            )

            # Agregar totais por status
            total_novos = sum(
                level_data.get("Novo", 0) for level_data in metrics_by_level.values()
            )
            total_pendentes = sum(
                level_data.get("Pendente", 0)
            ...
            trends = {"novos": 0, "pendentes": 0, "progresso": 0, "resolvidos": 0}
            ...
            timestamp = datetime.datetime.now(tz=datetime.timezone.utc).isoformat()
            self.logger.info(
                f"[{timestamp}] Métricas obtidas com sucesso - Filtro modificação, Total: {sum(result['totals'].values())}"
            )
    ...
    def _get_general_metrics_by_modification_date(
    ...
    def _get_metrics_by_level_by_modification_date(
        self, start_date: str, end_date: str
    ) -> dict:
