...
class TendenciasMetrics(BaseModel):
    ...
    novos: float = Field(
        default=0.0, description="Variação percentual de tickets novos"
    )
    pendentes: float = Field(
        default=0.0, description="Variação percentual de tickets pendentes"
    )
    progresso: float = Field(
        default=0.0, description="Variação percentual de tickets em progresso"
    )
    resolvidos: float = Field(
        default=0.0, description="Variação percentual de tickets resolvidos"
    )
...
class FiltersApplied(BaseModel):
    ...
    level: Optional[str] = Field(
        None, description="Nível do ticket (N1, N2, N3, N4)"
    )
    technician: Optional[str] = Field(
        None, description="ID ou nome do técnico"
    )
...
class DashboardMetrics(BaseModel):
    ...
    def validate_total_consistency(cls, v, values):
        ...
        if ('novos' in values and 'pendentes' in values and
                'progresso' in values and 'resolvidos' in values):
            calculated_total = (
                values['novos'] + values['pendentes'] +
                values['progresso'] + values['resolvidos']
            )
            if v != calculated_total:
                raise ValueError(
                    f'Total ({v}) deve ser igual à soma dos campos '
                    f'({calculated_total})'
                )
        return v
...
class TechnicianRanking(BaseModel):
    ...
    ticket_count: int = Field(
        ge=0, description="Quantidade de tickets atribuídos"
    )
...
class NewTicket(BaseModel):
    ...
    title: str = Field(
        min_length=1, max_length=255, description="Título do ticket"
    )
    ...
    def validate_priority(cls, v):
        ...
        if v not in valid_priorities:
            raise ValueError(
                f'Prioridade deve ser uma de: {", ".join(valid_priorities)}'
            )
        return v
...
class ApiResponse(BaseModel):
    ...
    errors: Optional[List[str]] = Field(
        None, description="Lista de erros, se houver"
    )
...
class ApiError(BaseModel):
    ...
    error_code: Optional[str] = Field(
        None, description="Código específico do erro"
    )
