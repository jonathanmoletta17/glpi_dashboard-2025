...
class GLPIMetricsCollector:
    ...
    def login(self) -> bool:
        ...
        """
        print(f"{Fore.CYAN}🔐 Iniciando autenticação no GLPI...{Style.RESET_ALL}")
        ...
        try:
            ...
            if self.session_token:
                ...
                print(f"{Fore.GREEN}✅ Autenticação realizada com sucesso!{Style.RESET_ALL}")
    ...
    def _get_all_active_technicians(self) -> List[Dict[str, Any]]:
        ...
        MÉTODO ALTERNATIVO: Buscar técnicos através de tickets atribuídos dos últimos 90 dias
        Isso contorna o problema da tabela Profile_User vazia e grupos 89-92 vazios
        
        Returns:
            Lista de técnicos com ID e nome
        """
        print("      Buscando técnicos ativos via tickets atribuídos (método alternativo do backend)...")
        ...
        params = {
            ...
            print(f"      Encontrados {len(tickets)} tickets dos últimos 90 dias")
            
            # Coletar IDs únicos de técnicos
            technician_ids = set()
            for ticket in tickets:
                if isinstance(ticket, dict):
                    tech_field = ticket.get('5')  # Campo técnico atribuído
                    tech_id = self._parse_technician_id(tech_field)
                    if tech_id:
                        technician_ids.add(tech_id)
            
            print(f"      Técnicos únicos com tickets: {len(technician_ids)}")
            
            # Buscar detalhes de cada técnico
            tecnicos_ativos = []
...
def main():
    ...
    print(f"{Fore.CYAN}Documentação viva para coleta de métricas GLPI{Style.RESET_ALL}\n")
